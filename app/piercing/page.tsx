...
function List({ title, items }: { title: string; items: { id: string; name: string }[] }) {
  return (
    <div className="border border-border bg-surface/40 p-6">
      <h2 className="font-serif text-2xl text-text">{title}</h2>
      <ul className="mt-4 grid gap-2 sm:grid-cols-2">
        {items.map((item) => (
          <li key={item.id} className="text-sm text-muted">{item.name}</li>
        ))}
      </ul>
    </div>
  );
...
export default function PiercingPage() {
  const faqs = faqItems.filter((item) => ["piercing", "randevu", "fiyatlandirma"].includes(item.category));
  return (
    <>
      <JsonLd data={serviceJsonLd({ name: "Tokat Piercing", description: "Kulak, yüz ve vücut piercing uygulamaları", path: "/piercing" })} />
      ...
      <PageHero
        ...
        crumbs={[{ name: "Ana Sayfa", path: "/" }, { name: "Piercing", path: "/piercing" }]}
      />
      ...
      <section className="py-14">
        <div className="container-page">
          ...
          <div className="mt-8 max-w-3xl">
            <FAQAccordion items={faqs} sourcePage="/piercing" />
          </div>
        </div>
      </section>
